...
/**
 * @brief Configuration for built-in ReAct Agent nodes.
 */
typedef struct {
    const char* model;          /**< AI model identifier. */
    const char* system_prompt;  /**< System prompt for ReAct reasoning. */
    const char* prompt;         /**< User prompt template. */
    int         max_iterations; /**< Max ReAct loop iterations (default 10). */
    double      temperature;
    int         max_tokens;
} csilk_agent_react_config_t;

/**
 * @brief Evaluator function signature for Reflexion nodes.
 * @param output Output text generated by the agent.
 * @param feedback_out Allocated pointer to store feedback string if evaluation fails.
 * @param user_data User data pointer.
 * @return 1 if output passes evaluation, 0 if it fails and needs reflection.
 */
typedef int (*csilk_reflexion_eval_fn)(const char* output, char** feedback_out, void* user_data);

/**
 * @brief Configuration for built-in Reflexion Agent nodes.
 */
typedef struct {
    const char*             model;           /**< AI model identifier. */
    const char*             prompt;          /**< Initial task prompt. */
    int                     max_reflections; /**< Max reflection iterations (default 3). */
    csilk_reflexion_eval_fn eval_fn;         /**< Custom output evaluation callback. */
    void*                   eval_user_data;  /**< Data for evaluation callback. */
} csilk_agent_reflexion_config_t;
...
/**
 * @brief Set a short-term memory key-value pair in the workflow context.
 * @param ctx   Execution context.
 * @param key   Memory key string.
 * @param value Memory value string.
 */
void csilk_wf_ctx_set_memory(csilk_wf_ctx_t* ctx, const char* key, const char* value);
...
/**
 * @brief Add a built-in ReAct Agent reasoning loop node.
 * @param wf     Workflow handle.
 * @param id     Unique node ID.
 * @param config ReAct Agent configuration (copied internally).
 * @return Node handle.
 */
csilk_wf_node_t*
...
/**
 * @brief Add a built-in Reflexion (Self-Correction) Agent node.
 * @param wf     Workflow handle.
 * @param id     Unique node ID.
 * @param config Reflexion Agent configuration (copied internally).
 * @return Node handle.
 */
csilk_wf_node_t* csilk_wf_add_agent_reflexion(csilk_wf_t*                           wf,
                                              const char*                           id,
...
/**
 * @brief Human Approval decision for HITL Agent nodes.
 */
typedef enum {
    CSILK_HITL_APPROVED,
    CSILK_HITL_REJECTED,
    CSILK_HITL_REQUIRES_HUMAN
} csilk_hitl_decision_t;

/**
 * @brief Evaluator function for Agent Human-In-The-Loop nodes.
 * @param output Output text generated by the agent.
 * @param user_data User context pointer.
 * @return CSILK_HITL_APPROVED if safe to auto-proceed, or CSILK_HITL_REQUIRES_HUMAN if human pause is needed.
 */
typedef csilk_hitl_decision_t (*csilk_agent_hitl_eval_fn)(const char* output, void* user_data);

/**
 * @brief Configuration for built-in Agent HITL (Human-in-the-Loop) nodes.
 */
typedef struct {
    const char*              model;          /**< AI model identifier. */
    const char*              prompt;         /**< Prompt template. */
    csilk_agent_hitl_eval_fn eval_fn;        /**< Risk / Approval evaluation callback. */
    void*                    eval_user_data; /**< User context for eval_fn. */
} csilk_agent_hitl_config_t;

/**
 * @brief Add a built-in Agent HITL (Human-in-the-Loop) node to the workflow.
 * @param wf     Workflow handle.
 * @param id     Unique node ID.
 * @param config HITL configuration (copied internally).
 * @return Node handle.
 */
csilk_wf_node_t*
