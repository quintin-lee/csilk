...
csilk_data_t* brainstorm_handler(csilk_wf_ctx_t* ctx, csilk_data_t* input,
                                 void* user_data) {
  (void)user_data;
...
csilk_data_t* writer_handler(csilk_wf_ctx_t* ctx, csilk_data_t* input,
                             void* user_data) {
  (void)user_data;
...
csilk_data_t* critic_handler(csilk_wf_ctx_t* ctx, csilk_data_t* input,
                             void* user_data) {
  (void)user_data;
  g_critique_count++;
  ...
  return csilk_wf_data_new(ctx, "critique",
                           csilk_wf_strdup(ctx, "ACCEPT: Excellent."));
}
...
csilk_data_t* fallback_handler(csilk_wf_ctx_t* ctx, csilk_data_t* input,
  ...
  return csilk_wf_data_new(
      ctx, "text/plain",
      csilk_wf_strdup(ctx, "Fallback content (offline mode)."));
}
...
void on_workflow_done(csilk_data_t* result) {
  ...
  if (result && result->value) {
    ...
    printf(
        "Workflow finished without final result (AI node likely failed without "
        "fallback).\n");
  }
}
...
int main() {
  ...
  /* 1. Define Nodes */
  csilk_wf_node_t* n_brain =
      csilk_wf_add(wf, "brainstorm", brainstorm_handler, NULL);
  ...
  csilk_wf_node_t* n_format = csilk_wf_add_ai(
      ...
      &(csilk_ai_config_t){
          .model = "gpt-4",
          .prompt = "Format this draft into technical markdown: "
