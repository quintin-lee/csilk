 ...
 */
void ws_on_message(csilk_ctx_t* c, const uint8_t* payload, size_t len,
                   int opcode) {
...
void ping_handler(csilk_ctx_t* c) { csilk_string(c, CSILK_STATUS_OK, "pong"); }

/** @brief Static file handler — serves files from the ./public directory. */
void static_handler(csilk_ctx_t* c) { csilk_static(c, "./public"); }

int main() {
  // Initialize logger
  csilk_log_config_t log_cfg = {.level = CSILK_LOG_DEBUG,
                                .file_path = NULL,
                                .max_file_size = 0,
                                .use_colors = -1};
  ...
  printf(
      "Test with: curl -H 'Authorization: secret-token' "
