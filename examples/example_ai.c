...
static void on_async_chat(int status, csilk_ai_chat_response_t* res,
                          void* user_data) {
  (void)user_data;
  if (status == 0) {
...
int main() {
  ...
  if (!api_key) {
    printf(
        "Please set AGENT_API_KEY environment variable to run this example.\n");
    return 0;
  }
  ...
  if (csilk_ai_chat(ai, &req, &res) == 0) {
    ...
    printf("\nUsage: prompt=%d, completion=%d, total=%d\n\n", res.prompt_tokens,
           res.completion_tokens, res.total_tokens);
    csilk_ai_chat_response_free(&res);
  } else {
  ...
  if (csilk_ai_embeddings(ai, "text-embedding-3-small", emb_input, 2, &eres) ==
      0) {
    ...
    if (eres.count > 0 && eres.dimension > 0) {
      printf("First 5 values of first vector: [%.4f, %.4f, %.4f, %.4f, %.4f]\n",
             eres.values[0], eres.values[1], eres.values[2], eres.values[3],
             eres.values[4]);
    }
  ...
  cJSON_AddStringToObject(loc, "description",
