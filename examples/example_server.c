...
void
openapi_spec_handler(csilk_ctx_t* c)
{
	if (g_router) {
...
int
mock_auth_validator(const char* token)
{
...
protected_handler(csilk_ctx_t* c)
{
	...
	csilk_string(
	    c, CSILK_STATUS_OK, "This is a protected resource. You have valid credentials.");
}
...
void
request_id_middleware(csilk_ctx_t* c)
{
...
void
request_timer_middleware(csilk_ctx_t* c)
{
	clock_t start = clock();
	csilk_next(c);
	clock_t end = clock();
...
main(int argc, char* argv[])
{
	...
	csilk_router_add_extended(router,
				  "POST",
				  "/login",
				  login_handlers,
				  1,
	...
	printf("  GET  /protected           Auth-protected (Authorization: "
	       "secret123)\n");
