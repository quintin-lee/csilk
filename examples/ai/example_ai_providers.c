...
main()
{
    ...
    if (echo) {
        csilk_ai_message_t msgs[] = {
            {"user", "Hello from csilk!"    },
            {"user", "Custom provider demo."}
        };
        csilk_ai_chat_request_t  req = {.model = "echo-v1", .messages = msgs, .message_count = 2};
    ...
    if (openai_key) {
        ...
        if (openai) {
            ...
            csilk_ai_chat_request_t req = {
                .model = openai_model, .messages = msgs, .message_count = 2, .max_tokens = 100};
            csilk_ai_chat_response_t res;
