...
static void* mock_ai_init(const char* api_key, const char* base_url) {
  (void)api_key;
  (void)base_url;
  return calloc(1, sizeof(mock_ai_state_t));
}

static int mock_ai_chat(void* state, const csilk_ai_chat_request_t* req,
...
static int mock_ai_embeddings(void* state, const char* model,
...
static void mock_ai_free(void* state) { free(state); }
...
static void generate_db_traffic(uv_timer_t* h) {
  (void)h;
  if (!global_db_pool) return;
...
static void generate_ai_traffic(uv_timer_t* h) {
  (void)h;
  if (!g_mock_ai) return;
...
static size_t curl_write_null(void* ptr, size_t size, size_t nmemb,
                              void* userdata) {
  (void)ptr;
  (void)userdata;
  return size * nmemb;
...
static void http_after_cb(uv_work_t* req, int status) {
  (void)status;
  free(req->data);
  free(req);
}

static void generate_http_traffic(uv_timer_t* h) {
  (void)h;
...
static void generate_security_traffic(uv_timer_t* h) {
  (void)h;
...
int main() {
  ...
  if (pool) {
    ...
    csilk_db_exec(
        pool,
        "INSERT OR IGNORE INTO users (name) VALUES ('Dashboard Demo User')");
    global_db_pool = pool;
  ...
  /* Cleanup */
  if (g_mock_ai) csilk_ai_free(g_mock_ai);
