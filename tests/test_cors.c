...
test_cors_basic()
{
    ...
    assert(strcmp(get_response_header(ctx, "Access-Control-Allow-Headers"), "Content-Type") == 0);
...
test_cors_options_preflight()
{
    ...
    csilk_cors_config_t config = {.allow_origin = "http://example.com", .allow_methods = "POST"};

    csilk_cors_middleware(ctx, &config);

    assert(csilk_get_status(ctx) == CSILK_STATUS_NO_CONTENT);
    assert(strcmp(get_response_header(ctx, "Access-Control-Allow-Origin"), "http://example.com") ==
           0);
