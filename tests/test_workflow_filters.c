...
static int mock_chat(void* state, const csilk_ai_chat_request_t* req,
  ...
  for (size_t i = 0; i < req->message_count; i++) {
    if (strcmp(req->messages[i].role, "user") == 0) {
      strncpy(g_final_prompt, req->messages[i].content,
              sizeof(g_final_prompt) - 1);
      break;
    }
  }
...
csilk_data_t* producer_handler(csilk_wf_ctx_t* ctx, csilk_data_t* input,
                               void* user_data) {
  (void)input;
  (void)user_data;
  return csilk_wf_data_new(ctx, "text",
                           csilk_wf_strdup(ctx, "  hello world  "));
}
