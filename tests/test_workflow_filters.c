...
csilk_data_t* producer_handler(csilk_wf_ctx_t* ctx, csilk_data_t* input, void* user_data) {
    (void)input; (void)user_data;
    return csilk_wf_data_new(ctx, "text", csilk_wf_strdup(ctx, "  hello world  "));
}

void on_filter_done(csilk_data_t* result) {
    (void)result;
    g_done = 1;
}

void test_workflow_template_filters() {
    ...
    csilk_wf_node_t* n2 = csilk_wf_add_ai(wf, "agent", &(csilk_ai_config_t){
        .prompt = "CMD: {{prod.value | trim | upper | summarize:5}}"
    });
