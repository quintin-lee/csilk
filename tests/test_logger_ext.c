...
int main() {
  ...
  printf(
      "Testing _csilk_log_structured with text format (extra discarded)...\n");
  {
    cfg.json_format = 0;
    cfg.file_path = NULL;
    int ret = csilk_log_init(cfg);
    ...
    _csilk_log_structured(CSILK_LOG_WARN, __FILE__, __LINE__, __func__, extra,
  ...
  printf("Testing _csilk_log_structured with NULL extra...\n");
  {
    ...
    int ret = csilk_log_init(cfg);
    assert(ret == 0);
    _csilk_log_structured(CSILK_LOG_ERROR, __FILE__, __LINE__, __func__, NULL,
                          "no extra fields");
    csilk_log_close();
  }
  ...
  printf("Testing file logging with rotation...\n");
  {
    ...
    int ret = csilk_log_init(cfg);
    assert(ret == 0);
    for (int i = 0; i < 50; i++) CSILK_LOG_I("test rotation line %d", i);
    csilk_log_close();
