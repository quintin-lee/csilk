...
csilk_data_t* slow_handler(csilk_wf_ctx_t* ctx, csilk_data_t* input, void* user_data) {
...
csilk_data_t* timeout_fallback_handler(csilk_wf_ctx_t* ctx, csilk_data_t* input, void* user_data) {
    (void)ctx; (void)input; (void)user_data;
...
void on_timeout_complete(csilk_data_t* result) {
    (void)result;
    g_done = 1;
}

void test_workflow_node_timeout() {
    printf("Testing per-node timeout...\n");
    g_node1_ran = 0; g_fallback_ran = 0; g_done = 0;
    ...
    // Node 1 might still finish in the thread pool, but its output should have been ignored.
    
    csilk_wf_free(wf);
    printf("test_workflow_node_timeout: PASS\n");
}

void test_workflow_global_ttl() {
    printf("Testing global workflow TTL...\n");
    g_node2_ran = 0; g_done = 0;
    ...
    for (int i = 0; i < 5; i++) {
        char id[16]; snprintf(id, sizeof(id), "node%d", i);
        csilk_wf_node_t* n = csilk_wf_add(wf, id, slow_handler, NULL);
        if (last) csilk_wf_bind(last, n);
        last = n;
    }
