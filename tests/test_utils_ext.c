...
static void test_sha256_multi_update() {
...
static void test_sha256_large() {
  ...
  uint8_t expected[] = {0x70, 0xd3, 0xbf, 0x8b, 0x0b, 0x9d, 0x83, 0xa6,
                        0x10, 0x12, 0xf3, 0x5f, 0xbf, 0x46, 0x0c, 0x42,
                        0x07, 0x06, 0x3f, 0xe3, 0x1b, 0x4d, 0x61, 0x78,
                        0x39, 0x0f, 0xe3, 0xb7, 0x21, 0xcc, 0x03, 0xf7};
  assert(memcmp(digest, expected, 32) == 0);
  printf("csilk_sha256 large passed!\n");
}

static void test_hmac_sha256_rfc4231() {
  printf("Testing HMAC-SHA256 RFC 4231 test case 2...\n");
  uint8_t out[32];
  const char* key = "Jefe";
  const char* data = "what do ya want for nothing?";

  csilk_hmac_sha256((uint8_t*)key, strlen(key), (uint8_t*)data, strlen(data),
                    out);
...
static void test_hmac_sha256_long_key() {
...
static void test_base64url_encode() {
...
static void test_base64url_encode_binary() {
...
static void test_base64url_decode_urlsafe() {
...
static void test_base64url_decode_invalid() {
...
static void test_csilk_hmac_sha256() {
...
static void test_hmac_driver_cb(const uint8_t* key, size_t klen,
...
static void test_csilk_hmac_sha256_with_crypto_driver() {
...
static void test_uuid_driver_cb(char* buf) {
  uuid_driver_called = 1;
  strcpy(buf, "driver-uuid-test-1234567890");
}

static void test_csilk_generate_uuid_with_driver() {
