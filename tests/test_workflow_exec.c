...
csilk_data_t* step1_handler(csilk_wf_ctx_t* ctx, csilk_data_t* input,
                            void* user_data) {
  (void)user_data;
  printf("Executing Step 1: %s\n", (char*)input->value);
  g_steps_completed++;

  return csilk_wf_data_new(ctx, "text/plain",
                           csilk_wf_strdup(ctx, "result from step 1"));
}

csilk_data_t* step2_handler(csilk_wf_ctx_t* ctx, csilk_data_t* input,
                            void* user_data) {
  (void)user_data;
  ...
  return csilk_wf_data_new(ctx, "text/plain",
                           csilk_wf_strdup(ctx, "result from step 2"));
}
