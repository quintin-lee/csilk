...
static void
set_request_header(csilk_ctx_t* c, const char* key, const char* value)
{
...
test_session_resume(void)
{
	...
	char cookie_buf[256];
	snprintf(cookie_buf, sizeof(cookie_buf), "csilk_session=%.*s", (int)id_len, eq);
...
static void
test_session_get_no_session(void)
{
	csilk_session_init();
	csilk_ctx_t c = {0};
	c.arena = csilk_arena_new(256);
...
static void
test_session_set_get_null(void)
{
	csilk_session_init();
	csilk_ctx_t c = {0};
	c.arena = csilk_arena_new(256);
