...
static int connect_server(const char* req, char* buf, size_t bufsize) {
    ...
    int n = 0;
    if (ret > 0) n = recv(sock, buf, bufsize - 1, 0);
    if (n > 0) buf[n] = '\0';
    close(sock);
    return n;
}
...
int main() {
    ...
    fflush(stdout);
    n = connect_server("GET / HTTP/1.1\r\nHost: localhost\r\n: value\r\nConnection: close\r\n\r\n", buf, sizeof(buf));
    if (n <= 0) { printf("PASS (connection closed as expected)\n"); passed++; }
    else { printf("FAIL (got response when expecting close)\n"); failed++; }

    // Test 7: Very long URL (within limits)
    printf("Test 7: Long URL ... ");
    fflush(stdout);
    char long_url[2048];
    int pos = snprintf(long_url, sizeof(long_url),
        "GET /search?");
