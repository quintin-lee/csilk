...
static void
mock_set(csilk_ctx_t* c, const char* key, void* value)
{
	(void)c;
	(void)key;
	(void)value;
...
static void*
mock_get(csilk_ctx_t* c, const char* key)
{
	(void)c;
	(void)key;
	get_called++;
...
static void
mock_clear(csilk_ctx_t* c)
{
	(void)c;
	clear_called++;
}

static csilk_storage_driver_t mock_driver = {.set = mock_set, .get = mock_get, .clear = mock_clear};
