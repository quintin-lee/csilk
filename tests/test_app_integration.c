...
static int read_response(int sock, char* buf, size_t size) {
  fd_set fds;
...
static void test_csilk_docs_static(void) {
  int sock = connect_server();
  if (sock < 0) {
  ...
  const char* req =
      "GET /csilk-docs/index.css HTTP/1.1\r\nHost: localhost\r\nConnection: "
      "close\r\n\r\n";
  send_request(sock, req);
  char buf[BUFSIZE] = {0};
  int n = read_response(sock, buf, sizeof(buf));
  ...
  test_result("GET /csilk-docs/index.css (status 200)",
              expect_status(buf, 200));
  test_result("GET /csilk-docs/index.css (CSS content)",
              expect_body(buf, "html"));
}

static void test_openapi_disabled(void) {
  ...
  test_result("GET /openapi.json disabled (status 404)",
              expect_status(buf, 404));
}
