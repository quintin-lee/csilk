...
csilk_data_t* json_producer_handler(csilk_wf_ctx_t* ctx, csilk_data_t* input,
                                    void* user_data) {
  (void)input;
  (void)user_data;
  printf("[Producer] Producing JSON data\n");
  const char* json =
      "{\"user\": {\"name\": \"Quintin\", \"details\": {\"age\": 30, "
      "\"active\": true}}}";
...
void test_workflow_jsonpath_injection() {
  ...
  csilk_wf_node_t* n1 =
      csilk_wf_add(wf, "producer", json_producer_handler, NULL);

  csilk_wf_node_t* n2 = csilk_wf_add_ai(
      wf, "consumer",
      &(csilk_ai_config_t){.prompt =
                               "Hello {{producer.value.user.name}}! Age: "
