...
csilk_data_t* json_producer_handler(csilk_wf_ctx_t* ctx, csilk_data_t* input, void* user_data) {
    (void)input; (void)user_data;
    printf("[Producer] Producing JSON data\n");
    const char* json = "{\"user\": {\"name\": \"Quintin\", \"details\": {\"age\": 30, \"active\": true}}}";
...
void on_jsonpath_complete(csilk_data_t* result) {
...
void test_workflow_jsonpath_injection() {
    printf("Testing JSONPath variable injection...\n");
    g_done = 0;
    if (g_final_output) { free(g_final_output); g_final_output = NULL; }
    ...
    csilk_wf_node_t* n2 = csilk_wf_add_ai(wf, "consumer", &(csilk_ai_config_t){
        .prompt = "Hello {{producer.value.user.name}}! Age: {{producer.value.user.details.age}}, Active: {{producer.value.user.details.active}}"
    });
    ...
    csilk_wf_free(wf);
    if (g_final_output) free(g_final_output);
