...
void test_workflow_template() {
    printf("Testing template resolution...\n");
    csilk_wf_t* wf = csilk_wf_new("temp_wf");
    
    // We need a dummy context to test internal resolve_templates if it was public,
    // but since it's static, we test it via a handler that returns the resolved string.
