...
{
	printf("Testing CORS with NULL config and context...\n");
...
test_cors_allow_credentials()
{
	...
	assert(allow_creds != NULL && strcmp(allow_creds, "true") == 0);
...
test_cors_non_wildcard_origin()
{
	...
	assert(allow_origin != NULL && strcmp(allow_origin, "http://example.com") == 0);
