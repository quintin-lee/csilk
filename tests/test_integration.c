...
static void test_get_root() {
  ...
  test_result("GET / (body 'Hello, World!')",
              expect_body(buf, "Hello, World!"));
}
...
static void test_get_api_data() {
  ...
  test_result("GET /api/data (status 200)",
              expect_status(buf, CSILK_STATUS_OK));
  test_result("GET /api/data (JSON body)",
              expect_body(buf, "\"key\":\"value\""));
...
static void test_post_login_ok() {
  ...
  test_result("POST /login ok (status 200)",
              expect_status(buf, CSILK_STATUS_OK));
  test_result("POST /login ok (token)",
              expect_body(buf, "\"token\":\"test123\""));
}
...
static void test_post_login_bad() {
  ...
  test_result("POST /login bad (status 401)",
              expect_status(buf, CSILK_STATUS_UNAUTHORIZED));
  test_result("POST /login bad (body)",
              expect_body(buf, "Invalid credentials"));
}
...
static void test_get_protected_ok() {
  ...
  test_result("GET /protected ok (status 200)",
              expect_status(buf, CSILK_STATUS_OK));
...
static void test_get_not_found() {
  ...
  test_result("GET /nonexistent (status 404)",
              expect_status(buf, CSILK_STATUS_NOT_FOUND));
}
...
static void test_keepalive() {
  ...
  char buf[BUFSIZE] = {0};
  int n = recv_response(sock, buf, sizeof(buf));
  int ok1 = n > 0 && expect_status(buf, CSILK_STATUS_OK) &&
            expect_body(buf, "Hello, World!");

  memset(buf, 0, sizeof(buf));
  send_request(sock, req2);
  n = recv_response(sock, buf, sizeof(buf));
  int ok2 = n > 0 && expect_status(buf, CSILK_STATUS_OK) &&
            expect_body(buf, "Echo: foo");
