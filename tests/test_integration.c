...
test_websocket_handshake()
{
    ...
    test_result("WS handshake (status 101)", expect_status(buf, CSILK_STATUS_SWITCHING_PROTOCOLS));
    test_result("WS handshake (Upgrade: websocket)", strstr(buf, "Upgrade: websocket") != nullptr);
...
test_ratelimit_integration()
{
    ...
    printf("DEBUG: buf3:\n%s\n", buf3);
    test_result("RateLimit 3 (status 429)", expect_status(buf3, CSILK_STATUS_TOO_MANY_REQUESTS));
