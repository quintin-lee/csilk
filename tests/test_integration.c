...
static int recv_response(int sock, char* buf, size_t size) {
    fd_set fds;
...
static int expect_status(const char* resp, int expected) {
    char expected_str[32];
    snprintf(expected_str, sizeof(expected_str), "HTTP/1.1 %d", expected);
    return strstr(resp, expected_str) != NULL;
}

static int expect_body(const char* resp, const char* body) {
...
static int send_request(int sock, const char* req) {
...
static int test_validator(const char* token) {
...
/* ---- Tests ---- */
static void test_get_root() {
    int sock = connect_server();
    if (sock < 0) { test_result("GET / (connect)", 0); return; }
    ...
    test_result("GET / (status 200)", expect_status(buf, 200));
    test_result("GET / (body 'Hello, World!')", expect_body(buf, "Hello, World!"));
}

static void test_get_user_param() {
    int sock = connect_server();
    if (sock < 0) { test_result("GET /user/42 (connect)", 0); return; }
    ...
    test_result("GET /user/42 (status 200)", expect_status(buf, 200));
    test_result("GET /user/42 (body)", expect_body(buf, "User ID: 42"));
}

static void test_get_api_data() {
    int sock = connect_server();
    if (sock < 0) { test_result("GET /api/data (connect)", 0); return; }
    ...
    test_result("GET /api/data (status 200)", expect_status(buf, 200));
    test_result("GET /api/data (JSON body)", expect_body(buf, "\"key\":\"value\""));
    test_result("GET /api/data (num field)", expect_body(buf, "\"num\":42"));
}

static void test_post_login_ok() {
    int sock = connect_server();
    if (sock < 0) { test_result("POST /login ok (connect)", 0); return; }
    const char* req =
    ...
    test_result("POST /login ok (status 200)", expect_status(buf, 200));
    test_result("POST /login ok (token)", expect_body(buf, "\"token\":\"test123\""));
}

static void test_post_login_bad() {
    int sock = connect_server();
    if (sock < 0) { test_result("POST /login bad (connect)", 0); return; }
    const char* req =
        "POST /login HTTP/1.1\r\nHost: localhost\r\nContent-Type: application/json\r\n"
        "Content-Length: 39\r\nConnection: close\r\n\r\n"
    ...
    test_result("POST /login bad (status 401)", expect_status(buf, 401));
    test_result("POST /login bad (body)", expect_body(buf, "Invalid credentials"));
}

static void test_get_protected_ok() {
    int sock = connect_server();
    if (sock < 0) { test_result("GET /protected ok (connect)", 0); return; }
    ...
    test_result("GET /protected ok (status 200)", expect_status(buf, 200));
    test_result("GET /protected ok (body)", expect_body(buf, "Protected data"));
}

static void test_get_protected_unauth() {
    int sock = connect_server();
    if (sock < 0) { test_result("GET /protected unauth (connect)", 0); return; }
    const char* req = "GET /protected HTTP/1.1\r\nHost: localhost\r\nConnection: close\r\n\r\n";
    send_request(sock, req);
    char buf[BUFSIZE] = {0};
    int n = recv_response(sock, buf, sizeof(buf));
    ...
    test_result("GET /protected unauth (status 401)", expect_status(buf, 401));
}

static void test_get_not_found() {
    int sock = connect_server();
    if (sock < 0) { test_result("GET /nonexistent (connect)", 0); return; }
...
static void test_get_echo() {
    int sock = connect_server();
    if (sock < 0) { test_result("GET /echo (connect)", 0); return; }
    ...
    test_result("GET /echo (status 200)", expect_status(buf, 200));
    test_result("GET /echo (body)", expect_body(buf, "Echo: test"));
}

static void test_keepalive() {
    int sock = connect_server();
    if (sock < 0) { test_result("keep-alive (connect)", 0); return; }
    ...
    char buf[BUFSIZE] = {0};
    int n = recv_response(sock, buf, sizeof(buf));
    int ok1 = n > 0 && expect_status(buf, 200) && expect_body(buf, "Hello, World!");

    memset(buf, 0, sizeof(buf));
    send_request(sock, req2);
    n = recv_response(sock, buf, sizeof(buf));
    int ok2 = n > 0 && expect_status(buf, 200) && expect_body(buf, "Echo: foo");

    memset(buf, 0, sizeof(buf));
    send_request(sock, req3);
    n = recv_response(sock, buf, sizeof(buf));
    int ok3 = n > 0 && expect_status(buf, 404);
...
static void test_post_invalid_json() {
    int sock = connect_server();
    if (sock < 0) { test_result("POST /login bad json (connect)", 0); return; }
    const char* req =
        "POST /login HTTP/1.1\r\nHost: localhost\r\nContent-Type: application/json\r\n"
        "Content-Length: 11\r\nConnection: close\r\n\r\n"
        "not-json!!!";
    ...
    test_result("POST /login bad json (status 400)", expect_status(buf, 400));
}
