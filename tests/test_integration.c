...
static void test_get_protected_unauth() {
    ...
    test_result("GET /protected unauth (status 401)", expect_status(buf, CSILK_STATUS_UNAUTHORIZED));
}
...
static void test_post_invalid_json() {
    ...
    test_result("POST /login bad json (status 400)", expect_status(buf, CSILK_STATUS_BAD_REQUEST));
}
