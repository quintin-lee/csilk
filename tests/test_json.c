...
test_json_response()
{
    ...
    assert(strstr(body, "\"status\":\"ok\"") != nullptr);
...
test_json_error()
{
    ...
    const char* body = csilk_get_response_body(ctx, &len);
    assert(body != nullptr);
    assert(strstr(body, "\"error\"") != nullptr);
    assert(strstr(body, "Not found") != nullptr);
    csilk_test_ctx_free(ctx);

    ctx = csilk_test_ctx_new();
    csilk_json_error(ctx, CSILK_STATUS_INTERNAL_SERVER_ERROR, nullptr);
    ...
    csilk_json_error(nullptr, CSILK_STATUS_BAD_REQUEST, "should not crash");
