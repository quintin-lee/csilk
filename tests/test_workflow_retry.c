...
csilk_data_t* flakey_handler(csilk_wf_ctx_t* ctx, csilk_data_t* input, void* user_data) {
...
void on_retry_complete(csilk_data_t* result) {
    assert(result != NULL);
    assert(strcmp((char*)result->value, "success") == 0);
    g_done = 1;
}

void test_workflow_automatic_retries() {
    printf("Testing automatic node retries...\n");
    g_attempts = 0; g_done = 0;
