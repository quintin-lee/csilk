...
static void on_message(csilk_ctx_t* c, const uint8_t* payload, size_t len,
                       int opcode) {
  (void)c;
...
static void on_any_message(csilk_ctx_t* c, const uint8_t* payload, size_t len,
                           int opcode) {
  (void)c;
  (void)payload;
  (void)len;
...
static void test_handshake() {
  ...
  csilk_set_request_header(&ctx, "Sec-WebSocket-Key",
...
static void test_handshake_missing_key() {
...
static void test_parse_frame_basic() {
  printf("Testing WebSocket frame parse basic...\n");

  csilk_ctx_t ctx = {0};
  ctx.arena = csilk_arena_new(1024);
  ctx.on_ws_message = on_message;
  uint8_t frame[] = {0x81, 0x85, 0x00, 0x00, 0x00, 0x00,
                     'h',  'e',  'l',  'l',  'o'};
...
static void test_parse_frame_unmasked() {
...
static void test_parse_frame_fragmented() {
  ...
  messages_received = 0;
  ctx.on_ws_message = on_message;
  uint8_t frame[] = {0x01, 0x85, 0x00, 0x00, 0x00, 0x00,
                     'h',  'e',  'l',  'l',  'o'};
  csilk_ws_parse_frame(&ctx, frame, sizeof(frame));
  assert(messages_received == 1);
  messages_received = 0;
...
static void test_parse_frame_binary() {
  ...
  messages_received = 0;
  uint8_t frame[] = {0x82, 0x84, 0x00, 0x00, 0x00,
...
static void test_parse_frame_medium_payload() {
...
static void test_parse_frame_ping_pong_close() {
...
static void test_parse_frame_truncated() {
...
static void test_parse_frame_large_payload() {
