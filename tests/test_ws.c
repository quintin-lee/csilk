...
static void on_message(csilk_ctx_t* c, const uint8_t* payload, size_t len, int opcode) {
    (void)c;
...
static void on_any_message(csilk_ctx_t* c, const uint8_t* payload, size_t len, int opcode) {
    (void)c; (void)payload; (void)len;
    messages_received++;
}
...
static void test_handshake() {
    ...
    csilk_header_t* h = ctx.response.headers;
