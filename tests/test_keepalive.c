...
static int recv_resp(int sock, char* buf, size_t size) {
  fd_set fds;
...
int main() {
  ...
  const char* req2 =
      "GET /echo?name=keepalive HTTP/1.1\r\nHost: localhost\r\nConnection: "
      "close\r\n\r\n";
  ...
  int ok2 = (n2 > 0 && expect_status(buf2, 200) &&
             expect_body(buf2, "Echo: keepalive"));
  printf("  Request 2 (same connection): %s\n", ok2 ? "PASS" : "FAIL");
  if (!ok2) printf("    Response: %s\n", buf2);
  if (ok2)
    passed++;
  else
    failed++;
