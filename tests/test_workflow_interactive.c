...
csilk_data_t* normal_handler(csilk_wf_ctx_t* ctx, csilk_data_t* input, void* user_data) {
    (void)input; (void)user_data;
    g_node1_ran++;
    return csilk_wf_data_new(ctx, "text", csilk_wf_strdup(ctx, "step1_ok"));
}

csilk_data_t* interactive_handler(csilk_wf_ctx_t* ctx, csilk_data_t* input, void* user_data) {
    (void)user_data;
    g_node2_ran++;
...
void on_interactive_done(csilk_data_t* result) {
...
void test_workflow_human_in_the_loop() {
    printf("Testing human-in-the-loop (Interactive Nodes)...\n");
    g_node1_ran = 0; g_node2_ran = 0; g_done = 0;
