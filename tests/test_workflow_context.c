...
static int mock_chat(void* state, const csilk_ai_chat_request_t* req, csilk_ai_chat_response_t* res) {
    ...
    if ((int)req->message_count > g_max_msgs_seen) g_max_msgs_seen = (int)req->message_count;
...
void on_context_done(csilk_data_t* result) {
    (void)result;
    g_done = 1;
}

void test_workflow_context_truncation() {
    printf("Testing Agent context window truncation...\n");
    g_max_msgs_seen = 0; g_system_msg_preserved = 0; g_done = 0;
