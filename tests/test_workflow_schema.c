...
csilk_data_t* invalid_json_handler(csilk_wf_ctx_t* ctx, csilk_data_t* input,
                                   void* user_data) {
  (void)input;
  (void)user_data;
  printf("[Producer] Returning JSON missing 'required' field\n");
  return csilk_wf_data_new(ctx, "application/json",
                           csilk_wf_strdup(ctx, "{\"wrong\": \"field\"}"));
}

csilk_data_t* schema_fallback_handler(csilk_wf_ctx_t* ctx, csilk_data_t* input,
                                      void* user_data) {
  (void)ctx;
  (void)input;
  (void)user_data;
...
void on_schema_complete(csilk_data_t* result) {
  (void)result;
  g_done = 1;
}

void test_workflow_schema_validation() {
  ...
  csilk_wf_node_t* n1 =
      csilk_wf_add(wf, "invalid_node", invalid_json_handler, NULL);

  // Require "result" field
  csilk_wf_node_set_schema(n1, "{\"required\": [\"result\"]}");

  csilk_wf_node_t* nf =
      csilk_wf_add(wf, "fallback", schema_fallback_handler, NULL);
