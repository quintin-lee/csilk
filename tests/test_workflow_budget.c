...
/* --- Mock Handlers --- */
csilk_data_t* expensive_handler(csilk_wf_ctx_t* ctx, csilk_data_t* input,
                                void* user_data) {
  (void)input;
  (void)user_data;
  printf("[Test] Node 1 running (Cost: 100 tokens)\n");
  csilk_data_t* out =
      csilk_wf_data_new(ctx, "text", csilk_wf_strdup(ctx, "data1"));
...
csilk_data_t* skip_handler(csilk_wf_ctx_t* ctx, csilk_data_t* input,
                           void* user_data) {
  (void)ctx;
  (void)input;
  (void)user_data;
...
void on_budget_complete(csilk_data_t* result) {
...
void test_workflow_budget_enforcement() {
