...
/* --- Mock Tool --- */
char*
mock_get_weather(const char* args_json, void* user_data)
{
	(void)user_data;
...
static csilk_ai_driver_t mock_driver = {.name =
					    "openai", // Override openai for testing without network
					.init = mock_init,
					.chat = mock_chat,
					.free = mock_free};
...
test_workflow_autonomous_tools()
{
	...
	csilk_wf_node_t* n1 = csilk_wf_add_ai(
	    wf, "agent", &(csilk_ai_config_t){.model = "gpt-4", .prompt = "What is the weather?"});
