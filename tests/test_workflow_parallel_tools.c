...
/* --- Mock Tool --- */
char* slow_tool(const char* args_json, void* user_data) {
...
static int mock_chat(void* state, const csilk_ai_chat_request_t* req, csilk_ai_chat_response_t* res) {
    ...
    if (turn == 1) {
        ...
        for (int i = 0; i < 3; i++) {
            char id[16]; snprintf(id, sizeof(id), "call_%d", i);
...
void test_workflow_parallel_tools() {
    ...
    // If sequential, would take > 300ms. If parallel, should be ~100ms (+ overhead).
    // Thread pool size might limit parallelism, but for 3 tasks it should be fine.
    // We allow some buffer, say 250ms max.
