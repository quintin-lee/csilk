...
static void
test_size_max_near_boundary(void)
{
    printf("Running test_size_max_near_boundary...\n");
    csilk_arena_t* a = csilk_arena_new(4096);
    assert(a != NULL);
...
static void
test_max_total_bytes_enforced(void)
{
    printf("Running test_max_total_bytes_enforced...\n");
    csilk_arena_t* a = csilk_arena_new(128);
    assert(a != NULL);
    assert(csilk_arena_set_max_bytes(a, 256) == 0);
    // 100 bytes leaves 28 remaining — next 100 forces new chunk
    void* p1 = csilk_arena_alloc(a, 100);
    assert(p1 != NULL); // chunk1, total=128
    void* p2 = csilk_arena_alloc(a, 100);
    assert(p2 != NULL); // chunk2, total=256
    void* p3 = csilk_arena_alloc(a, 100);
    assert(p3 == NULL); // would be 384>256
    csilk_arena_free(a);
    PASS();
}
static void
test_total_allocated_no_underflow(void)
{
    ...
    csilk_arena_reset(a);
    assert(csilk_arena_alloc(a, 1) != NULL);
    csilk_arena_free(a);
...
test_null_arena(void)
{
    ...
    csilk_arena_reset(NULL);
    csilk_arena_set_alignment(NULL, 1);
    assert(csilk_arena_set_max_bytes(NULL, 1024) == -1);
    size_t ts = 0, tu = 0;
    csilk_arena_get_stats(NULL, &ts, &tu);
    assert(csilk_arena_contains(NULL, (void*)0x1234) == 0);
    PASS();
}
static void
test_get_stats_no_overflow(void)
{
    printf("Running test_get_stats_no_overflow...\n");
    csilk_arena_t* a = csilk_arena_new(4096);
    assert(a != NULL);
    for (int i = 0; i < 1000; i++) {
        csilk_arena_alloc(a, 4000);
    }
    size_t ts = 0, tu = 0;
    csilk_arena_get_stats(a, &ts, &tu);
    assert(ts > 0);
    assert(tu > 0);
    assert(tu <= ts);
    assert(ts <= (size_t)1000 * 4096);
    csilk_arena_free(a);
    PASS();
}
static void
test_repeated_reset_tight_max_bytes(void)
{
...
static void
test_chunk_alignment_overflow_guard(void)
{
...
static void
test_64_align_across_resets(void)
{
...
static void
test_fast_path_uintptr_safety(void)
{
    printf("Running test_fast_path_uintptr_safety...\n");
    csilk_arena_t* a = csilk_arena_new(64);
    assert(a != NULL);
    for (int i = 0; i < 200; i++) {
        void* p = csilk_arena_alloc(a, 60);
        assert(p != NULL);
...
static void
test_max_bytes_cumulative(void)
{
    ...
    assert(csilk_arena_alloc(a, 900) ==
           NULL); // chunk4 would be 4096>3072  // 4th chunk would make total=4096>3072
    csilk_arena_free(a);
    PASS();
}
static void
test_free_underflow_guard(void)
{
...
static void
test_slow_path_many_chunks(void)
{
    printf("Running test_slow_path_many_chunks...\n");
    csilk_arena_t* a = csilk_arena_new(64);
    assert(a != NULL);
    for (int i = 0; i < 100; i++) {
        assert(csilk_arena_alloc(a, 60) != NULL);
    }
    size_t ts = 0, tu = 0;
    csilk_arena_get_stats(a, &ts, &tu);
    assert(ts > 0);
...
static void
test_calloc_overflow_guard(void)
{
...
static void
test_zero_size_alloc(void)
{
    printf("Running test_zero_size_alloc...\n");
    csilk_arena_t* a = csilk_arena_new(4096);
    assert(a != NULL);
    void* p = csilk_arena_alloc(a, 0);
    assert(p == (void*)(uintptr_t)1);
    p = csilk_arena_alloc(a, 64);
    assert(p != NULL);
    csilk_arena_free(a);
    PASS();
}
static void
test_max_bytes_unlimited(void)
{
...
static void
test_reset_reuses_same_address(void)
{
...
static void
test_custom_chunk_large_alignment(void)
{
    printf("Running test_custom_chunk_large_alignment...\n");
    csilk_arena_t* a = csilk_arena_new(8192);
    assert(a != NULL);
    csilk_arena_set_alignment(a, 1);
    for (int i = 0; i < 50; i++) {
        void* p = csilk_arena_alloc(a, 1);
