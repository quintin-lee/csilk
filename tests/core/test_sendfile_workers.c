...
static _Atomic(bool)* g_server_ready_ptr = NULL;

static void
on_server_started(csilk_ctx_t* c)
{
    (void)c;
    if (g_server_ready_ptr) {
        atomic_store_explicit(g_server_ready_ptr, true, memory_order_release);
    }
}
...
client_worker(void* raw_arg)
{
    ...
    for (int req_idx = 0; req_idx < arg->requests_per_client; req_idx++) {
        ...
        for (int retry = 0; retry < 3 && !req_success; retry++) {
