...
void test_mq_wal_persistence_recovery() {
    ...
    /* Phase 1: Publish messages and shut down */
    {
        ...
        /* Wait a bit for WAL to be written if it's async (though usually it's sync in this impl) */
        usleep(100000); 
    ...
    received_count = 0;
    {
        ...
        /* We expect 3 messages. Recovery usually queues them via uv_async or similar. */
        ...
        printf("Successfully recovered 3 messages: %s, %s, %s\n", 
               received_payloads[0], received_payloads[1], received_payloads[2]);
