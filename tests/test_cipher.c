...
static int my_symmetric_encrypt(const uint8_t* key, size_t key_len,
...
static int my_symmetric_decrypt(const uint8_t* key, size_t key_len,
...
static int my_generate_keypair(char* public_key, size_t* pub_len,
...
static void test_default_symmetric_roundtrip(void) {
  ...
  int r = _csilk_symmetric_encrypt(NULL, key, sizeof(key), plaintext, pt_len,
                                   iv, sizeof(iv), ciphertext, &ct_len, tag,
                                   sizeof(tag));
  assert(r == 0);
  ...
  r = _csilk_symmetric_decrypt(NULL, key, sizeof(key), ciphertext, ct_len, iv,
                               sizeof(iv), tag, sizeof(tag), decrypted,
                               &dec_len);
  assert(r == 0);
...
static void test_default_symmetric_wrong_tag(void) {
...
static void test_default_symmetric_bad_key(void) {
  ...
  int r = _csilk_symmetric_encrypt(NULL, key, sizeof(key),
                                   (const uint8_t*)"data", 4, iv, sizeof(iv),
...
static void test_default_asymmetric_roundtrip(void) {
  ...
  r = _csilk_asymmetric_encrypt(NULL, pub_key, pub_len, plaintext, pt_len,
                                ciphertext, &ct_len);
  assert(r == 0);
  ...
  r = _csilk_asymmetric_decrypt(NULL, priv_key, priv_len, ciphertext, ct_len,
                                decrypted, &dec_len);
  assert(r == 0);
  assert(dec_len == pt_len);
  assert(memcmp(decrypted, plaintext, pt_len) == 0);
...
static void test_default_sign_verify(void) {
...
static void test_custom_driver_pluggable(void) {
  ...
  int r = _csilk_symmetric_encrypt(&c, key, sizeof(key), pt, pt_len, iv,
                                   sizeof(iv), ct, &ct_len, tag, sizeof(tag));
  assert(r == 0);
  ...
  r = _csilk_symmetric_decrypt(&c, key, sizeof(key), ct, ct_len, iv, sizeof(iv),
                               tag, sizeof(tag), dec, &dec_len);
  assert(r == 0);
...
static void test_custom_keygen(void) {
...
static void test_null_context_defaults(void) {
  ...
  int r = _csilk_symmetric_encrypt(NULL, key, sizeof(key), pt, pt_len, iv,
                                   sizeof(iv), ct, &ct_len, tag, sizeof(tag));
  assert(r == 0);

  uint8_t dec[256];
  size_t dec_len = sizeof(dec);
  r = _csilk_symmetric_decrypt(NULL, key, sizeof(key), ct, ct_len, iv,
                               sizeof(iv), tag, sizeof(tag), dec, &dec_len);
  assert(r == 0);
  assert(dec_len == pt_len);
  assert(memcmp(dec, pt, pt_len) == 0);
