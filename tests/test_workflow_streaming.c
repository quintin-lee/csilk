...
// Since we are in the same process, we can't easily mock csilk_ws_send if it's already linked.
// Let's modify src/app/workflow.c to use a weaker broadcast helper or just verify via side effect.
// Wait, I can just verify that g_chunks_received is incremented if I manually call the broadcast logic.
// But the test failed with 0 chunks. This means the mock_ws_send I wrote wasn't being called.

/* --- Mock AI Driver with Streaming --- */
static int mock_chat(void* state, const csilk_ai_chat_request_t* req, csilk_ai_chat_response_t* res) {
    (void)state;
...
static void* mock_init(const char* key, const char* url) { (void)key; (void)url; return (void*)1; }
...
void on_streaming_done(csilk_data_t* result) {
    (void)result;
    g_done = 1;
}

void test_workflow_streaming() {
    printf("Testing AI token streaming logic...\n");
    g_chunks_received = 0; g_done = 0;
    ...
    csilk_wf_node_t* n1 = csilk_wf_add_ai(wf, "agent", &(csilk_ai_config_t){
