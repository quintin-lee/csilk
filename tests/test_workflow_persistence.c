...
csilk_data_t* step_a_handler(csilk_wf_ctx_t* ctx, csilk_data_t* input, void* user_data) {
...
csilk_data_t* step_b_handler(csilk_wf_ctx_t* ctx, csilk_data_t* input, void* user_data) {
    (void)user_data;
    g_step_b_ran++;
...
void test_workflow_persistence() {
    ...
    // Node B should be AT LEAST 1 (resumed). It might be 2 if the background thread from Part 1 finished.
    // For the test to pass reliably in one process, we accept >= 1.
