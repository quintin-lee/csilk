 ...
 *
 * The module provides both synchronous (blocking) and asynchronous (I/O
    ...
    while (retries <= max_retries) {
       ...
       rate limits (429), or temporary server unavailability (502/503). */
        if (res->error_message &&
            (strstr(res->error_message, "CURL error") || strstr(res->error_message, "429") ||
             strstr(res->error_message, "502") || strstr(res->error_message, "503"))) {
            retries++;
...
/** @brief Send a chat completion request asynchronously on the thread
 *  pool.
 *
 ...
 *  @note Thread-safe to call from any thread (the I/O backend queues the work).
                    ...
                    void*                          user_data)
{
    ...
    work->data = ar;
    CSILK_LOG_D("Queueing async chat request for model '%s'", req->model ? req->model : "default");
                    ...
                    csilk_ai_embeddings_response_t* res)
{
    ...
    CSILK_LOG_D("AI embeddings request initiated. Model: '%s', Inputs count: %zu", model, count);

    memset(res, 0, sizeof(*res));
    if (!ai->driver->embeddings) {
        res->error_message = strdup("Driver does not support embeddings");
        atomic_fetch_add(&ai_errors_total, 1);
        CSILK_LOG_E("Failed to generate AI embeddings: driver '%s' does not support embeddings",
                    ai->driver->name);
...
/** @brief Thread-pool work callback for async embeddings.
...
/** @brief After-work callback for async embeddings — delivers the
...
/** @brief Generate embeddings asynchronously on the thread pool.
 *
...
csilk_ai_free(csilk_ai_t* ai)
{
    ...
    CSILK_LOG_D("Freeing AI engine instance and driver state for driver '%s'", ai->driver->name);
...
csilk_ai_context_add(csilk_ai_context_t* ctx, const char* role, const char* content)
{
    ...
    if (ctx->max_history > 0 && ctx->count >= ctx->max_history) {
        CSILK_LOG_D("AI context sliding window reached limit (%zu). Evicting oldest message.",
                    ctx->max_history);
