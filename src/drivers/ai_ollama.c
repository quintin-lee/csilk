...
struct curl_response {
  ...
  size_t
      size; /**< Current length of data in body (excluding NUL terminator). */
};
...
static int ollama_chat(void* state_ptr, const csilk_ai_chat_request_t* req,
  ...
  if (resp_root) {
    ...
    res->prompt_tokens =
        cJSON_GetObjectItem(resp_root, "prompt_eval_count")->valueint;
    res->completion_tokens =
        cJSON_GetObjectItem(resp_root, "eval_count")->valueint;
