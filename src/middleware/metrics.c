...
csilk_process_get_stats(csilk_process_stats_t* stats)
{
    ...
    /* Get resident memory (RSS) from the I/O backend */
    csilk_io_resident_set_memory(&stats->rss_bytes);

    /* Get CPU time (User/System) from the I/O backend */
...
append_metric(char** buf, size_t* size, size_t* offset, const char* fmt, ...)
{
    ...
    if (*offset + n + 1 > *size) {
        ...
        if (!new_buf) {
            CSILK_LOG_E("Metrics: failed to expand metric buffer to %zu bytes", new_size);
            return;
        }
        *buf = new_buf;
        *size = new_size;
...
csilk_metrics_handler(csilk_ctx_t* c)
{
    ...
    append_metric(&buf, &size, &offset, "# TYPE http_request_duration_microseconds_agg counter\n");
    append_metric(&buf,
                  &size,
                  &offset,
    ...
    for (int i = 0; i < CSILK_METRICS_MAX_ENTRIES; i++) {
        ...
        if (atomic_load(&slot->in_use)) {
            ...
            append_metric(
                ...
                slot->status,
                slot->route,
                (unsigned long long)atomic_load(&slot->buckets[CSILK_METRICS_BUCKET_COUNT]));
        }
    }
