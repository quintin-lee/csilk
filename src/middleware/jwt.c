...
jwt_build_header(csilk_jwt_alg_t alg)
{
    ...
    /**
 * @brief Parse algorithm string from JWT header JSON.
 * @param header_json  Decoded JWT header JSON string.
 * @return csilk_jwt_alg_t or CSILK_JWT_NONE if invalid.
 */
    static csilk_jwt_alg_t jwt_parse_header_alg(const char* header_json)
    {
        if (!header_json) {
            return CSILK_JWT_NONE;
        }

        csilk_json_t* header = csilk_json_parse(header_json);
        if (!header) {
            return CSILK_JWT_NONE;
        }

        const char*     alg_str = csilk_json_get_string(header, "alg");
        csilk_jwt_alg_t alg = CSILK_JWT_NONE;
 ...
 */
    static char* jwt_generate_internal(csilk_ctx_t * c,
                                       ...
                                       csilk_jwt_alg_t algorithm)
    {
        ...
        if (algorithm == CSILK_JWT_HS256) {
            ...
            size_t  sig_len = (algorithm == CSILK_JWT_ES256) ? CSILK_ES256_SIGNATURE_SIZE
 ...
 */
    ...
    {
        return jwt_generate_internal(
            c, payload, secret, secret ? strlen(secret) : 0, CSILK_JWT_HS256);
    }
 ...
 */
    static csilk_json_t* jwt_verify_internal(csilk_ctx_t * c,
                                             const char*                token,
                                             const char*                key,
                                             size_t                     key_len,
                                             const csilk_jwt_options_t* options)
    {
        ...
        /* Verify algorithm matches expected to prevent alg confusion attacks.
     * Extract alg from JWT header and compare with expected algorithm. */
        ...
        if (header_b64) {
            ...
            uint8_t header_decoded[512];
            int     header_dec_len =
                csilk_base64url_decode(header_b64, header_decoded, sizeof(header_decoded));
            if (header_dec_len > 0 && header_dec_len < (int)sizeof(header_decoded)) {
                header_decoded[header_dec_len] = '\0';
                csilk_jwt_alg_t token_alg = jwt_parse_header_alg((char*)header_decoded);
                if (token_alg != CSILK_JWT_NONE && token_alg != algorithm) {
                    CSILK_LOG_W("JWT: Algorithm mismatch - token uses %s, expected %s",
                                jwt_alg_str(token_alg),
                                jwt_alg_str(algorithm));
                    free(header_b64);
                    return NULL;
                }
            }
 ...
 */
    csilk_json_t* csilk_jwt_verify_ex(csilk_ctx_t * c,
                                      const char*     token,
                                      const char*     key,
                                      size_t          key_len,
                                      csilk_jwt_alg_t algorithm)
 ...
 */
    void csilk_jwt_middleware_options(
        csilk_ctx_t * c, const char* key, size_t key_len, const csilk_jwt_options_t* options)
    {
        if (!c || !key) {
 ...
 */
    void csilk_jwt_middleware_ex(
        csilk_ctx_t * c, const char* key, size_t key_len, csilk_jwt_alg_t algorithm)
    {
        csilk_jwt_options_t opts = {
            .algorithm = algorithm,
            .flags = CSILK_JWT_NONE,
