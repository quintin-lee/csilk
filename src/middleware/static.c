...
set_range_response(csilk_ctx_t* c, int fd, size_t size, const char* mime_type)
{
    ...
    if (!range_hdr || strncmp(range_hdr, "bytes=", 6) != 0) {
        CSILK_LOG_T("Static: Serving full file (fd: %d, size: %zu, mime: %s)", fd, size, mime_type);
    ...
    if (!dash) {
        CSILK_LOG_W("Static: Range request unsatisfiable: missing dash (Range header: '%s')",
                    range_hdr);
    ...
    *dash = '-';
    ...
    snprintf(
        content_range, sizeof(content_range), "bytes %lld-%lld/%zu", range_start, range_end, size);
    CSILK_LOG_T(
...
/**
 * @brief Work callback: resolve and open the requested static file.
 *
 * Runs on the thread pool. Resolves the requested path relative to
...
/** @brief Work callback: open and stat a specific file.
 *
 * Runs in the thread pool. Opens the file specified by the
 ...
 * operations to the thread pool. The response is sent via the
 ...
 *
 * Offloads file open/stat operations to the thread pool. The response
