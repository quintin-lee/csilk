...
static const char* level_names[] = {"TRACE", "DEBUG", "INFO ",
                                    "WARN ", "ERROR", "FATAL"};
static const char* level_colors[] = {"\x1b[35m", "\x1b[36m", "\x1b[32m",
                                     "\x1b[33m", "\x1b[31m", "\x1b[41;1m"};
...
static int log_text(csilk_log_level_t lv, const char* file, int line,
  ...
  if (g_logger.config.use_colors)
    n += fprintf(g_logger.fp, "%s %s%s\x1b[0m [%s:%d] %s(): ", ts,
                 level_colors[lv], level_names[lv], fn, line, func);
  else
...
static cJSON* build_json_entry(csilk_log_level_t lv, const char* file, int line,
                               const char* func, const char* msg, int msg_len) {
  const char* fn = strrchr(file, '/');
  fn = fn ? fn + 1 : file;
...
static int log_json(csilk_log_level_t lv, const char* file, int line,
                    const char* func, cJSON* extra, const char* msg,
                    int msg_len) {
