...
csilk_server_run(csilk_server_t* server, int port)
{
    ...
    io_uring_sqe_set_data64(sig_sqe, uring_encode_data(URING_OP_WAKEUP, NULL, &server->sig_handle));
    ...
    if (server->mq && server->mq->async_handle.event_fd >= 0) {
        ...
        if (mq_sqe) {
            ...
            io_uring_sqe_set_data64(
                mq_sqe, uring_encode_data(URING_OP_WAKEUP, NULL, &server->mq->async_handle));
        }
    }
    ...
    if (server->worker_pools[0].thread_pool) {
        ...
        if (tp_sqe && tp_fd >= 0) {
            ...
            io_uring_sqe_set_data64(
                tp_sqe,
                uring_encode_data(URING_OP_WAKEUP, NULL, server->worker_pools[0].thread_pool));
        }
    }
    ...
    while (running) {
        ...
        if (op == URING_OP_ACCEPT) {
            ...
            if (acc_sqe) {
                io_uring_prep_accept(
                    acc_sqe, server->server_handle.fd, NULL, NULL, SOCK_NONBLOCK | SOCK_CLOEXEC);
                io_uring_sqe_set_data64(acc_sqe, uring_encode_data(URING_OP_ACCEPT, NULL, ptr));
            ...
            if (ptr == &server->async_handle) {
                ...
                if (poll_sqe) {
                    io_uring_prep_poll_add(poll_sqe, server->sig_handle.signal_fd, POLLIN);
                    io_uring_sqe_set_data64(poll_sqe,
                                            uring_encode_data(URING_OP_WAKEUP, NULL, ptr));
                    io_uring_submit(server->loop);
                }
                ...
                uint64_t val;
                if (read(server->worker_pools[0].dispatch_async.event_fd, &val, sizeof(val)) > 0) {
                    on_dispatch_async(&server->worker_pools[0].dispatch_async);
                }
                struct io_uring_sqe* poll_sqe = io_uring_get_sqe(server->loop);
                if (poll_sqe) {
                    io_uring_prep_poll_add(
                        poll_sqe, server->worker_pools[0].dispatch_async.event_fd, POLLIN);
                    io_uring_sqe_set_data64(poll_sqe,
                                            uring_encode_data(URING_OP_WAKEUP, NULL, ptr));
                    io_uring_submit(server->loop);
                }
                ...
                uint64_t val;
                if (read(server->mq->async_handle.event_fd, &val, sizeof(val)) > 0) {
                    if (server->mq->async_handle.cb) {
                        server->mq->async_handle.cb(&server->mq->async_handle);
                    }
                }
                struct io_uring_sqe* poll_sqe = io_uring_get_sqe(server->loop);
                if (poll_sqe) {
                    io_uring_prep_poll_add(poll_sqe, server->mq->async_handle.event_fd, POLLIN);
                    io_uring_sqe_set_data64(poll_sqe,
                                            uring_encode_data(URING_OP_WAKEUP, NULL, ptr));
                    io_uring_submit(server->loop);
                }
                ...
                uint64_t             val;
                uring_thread_pool_t* tp = (uring_thread_pool_t*)server->worker_pools[0].thread_pool;
                if (read(uring_tp_wakeup_fd(tp), &val, sizeof(val)) > 0) {
                    uring_tp_drain(tp);
                }
                int                  tp_fd = uring_tp_wakeup_fd(tp);
