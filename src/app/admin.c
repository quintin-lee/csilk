...
static void admin_stats_handler(csilk_ctx_t* c) {
  ...
  double avg_latency =
      total_req > 0 ? (double)total_dur / total_req / 1000.0 : 0;  // ms
  ...
  cJSON_AddNumberToObject(ai, "avg_duration",
                          ai_stats.requests_total > 0
                              ? (double)ai_stats.duration_us_total /
  ...
  cJSON_AddNumberToObject(sec, "rate_limit_blocks",
                          (double)sec_stats.rate_limit_blocks);
  cJSON_AddNumberToObject(sec, "csrf_violations",
                          (double)sec_stats.csrf_violations);
  cJSON_AddNumberToObject(sec, "auth_failures",
                          (double)sec_stats.auth_failures);
  ...
  cJSON_AddNumberToObject(proc, "rss_kb",
                          (double)proc_stats.rss_bytes / 1024.0);
 ...
 */
void csilk_admin_serve_secure(csilk_app_t* app, const char* app_path,
                              csilk_handler_t auth_middleware) {
  if (!app || !app_path) return;
