...
static void node_add_edge(csilk_wf_node_t* from, const char* condition,
                          csilk_wf_node_t* to, int is_loop) {
  if (!from || !to) return;
  ...
  from->edges[from->edge_count].condition =
      condition ? strdup(condition) : NULL;
...
void csilk_wf_register_monitor(csilk_wf_t* wf, csilk_ctx_t* c) {
  ...
  if (wf->monitor_count >= wf->monitor_capacity) {
    ...
    csilk_ctx_t** new_monitors =
        realloc(wf->monitors, sizeof(csilk_ctx_t*) * new_cap);
    if (new_monitors) {
...
static void _wf_broadcast(csilk_wf_t* wf, const char* event,
                          const char* node_id, const char* payload) {
...
static csilk_data_t* ai_node_handler(csilk_wf_ctx_t* ctx, csilk_data_t* input,
  ...
  while (iterations < 10) {
    ...
    if (res.tool_call_count > 0) {
      ...
      uv_cond_init(&c);
      sub_tool_work_t* sws =
          calloc(res.tool_call_count, sizeof(sub_tool_work_t));
      ...
      for (size_t i = 0; i < res.tool_call_count; i++) {
        ...
        reqs[i].data = &sws[i];
        uv_queue_work(ctx->wf->loop, &reqs[i], sub_worker_cb,
                      after_sub_worker_cb);
      }
    ...
    out =
        csilk_wf_data_new(ctx, "text/plain", csilk_wf_strdup(ctx, res.content));
...
char* csilk_wf_trace_to_json(const csilk_wf_trace_t* trace) {
  ...
  cJSON_AddNumberToObject(root, "duration_us",
                          (double)(trace->end_time - trace->start_time));
  ...
  for (size_t i = 0; i < trace->node_count; i++) {
    ...
    cJSON_AddNumberToObject(nj, "duration_us",
                            (double)(n->end_time - n->start_time));
...
static void after_worker_cb(uv_work_t* req, int status) {
  ...
  _wf_broadcast(ctx->wf, "node_finish", node->id,
                output ? (char*)output->value : NULL);
  ...
  if (triggered_count == 0 && current_active == 0) {
    ...
    _wf_broadcast(ctx->wf, "workflow_end", NULL,
                  output ? (char*)output->value : NULL);
...
static void execute_node(csilk_wf_ctx_t* ctx, csilk_wf_node_t* node,
  ...
  if (ctx->trace) {
    ...
    tn->input_dump =
        strdup(input && input->value ? (char*)input->value : "(null)");
    work->trace_node = tn;
  }
...
static const char* csilk_wf_run_ext_internal(
  ...
  if (!wf || wf->node_count == 0) {
    ...
    if (trace_cb) trace_cb(NULL, NULL);
    return NULL;
  }
