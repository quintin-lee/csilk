...
struct gin_group_s {
  ...
  gin_handler_t* middlewares;    /**< Middleware handlers array. */
  size_t middleware_count;       /**< Number of middleware handlers. */
  gin_group_t* parent;           /**< Parent group (NULL for root). */
};
...
void gin_group_use(gin_group_t* group, gin_handler_t handler) {
  ...
  gin_handler_t* new_middlewares =
      realloc(group->middlewares,
              (group->middleware_count + 1) * sizeof(gin_handler_t));
  if (new_middlewares) {
...
static int gather_handlers(gin_group_t* group, gin_handler_t** handlers,
  ...
  if (group->middleware_count > 0) {
    gin_handler_t* new_handlers = realloc(
        *handlers, (*count + group->middleware_count) * sizeof(gin_handler_t));
    if (!new_handlers) {
      return -1;
    }
    *handlers = new_handlers;
    memcpy(*handlers + *count, group->middlewares,
           group->middleware_count * sizeof(gin_handler_t));
...
void gin_group_add_route(gin_group_t* group, const char* method,
  ...
  gin_handler_t* new_handlers =
      realloc(handlers, (count + 1) * sizeof(gin_handler_t));
  if (!new_handlers) {
    free(full_path);
    free(handlers);
    return;
