...
void gin_router_add(gin_router_t *r, const char *method, const char *path, gin_handler_t *handlers, size_t handler_count) {
    ...
    while ((seg = get_next_segment(&p)) != NULL) {
        ...
        for (gin_router_node_t *child = curr->children; child != NULL; child = child->sibling) {
...
static gin_handler_t* match_node(gin_router_node_t *node, const char *method, const char *path, gin_ctx_t *ctx) {
    if (*path == '/') path++;
    ...
    for (gin_router_node_t *child = node->children; child != NULL; child = child->sibling) {
        if (child->type == GIN_NODE_STATIC) {
            if (strlen(child->segment) == seg_len && strncmp(child->segment, path, seg_len) == 0) {
...
int gin_router_match_ctx(gin_router_t *r, gin_ctx_t *c) {
    if (!c) return 0;
    c->params_count = 0;
    gin_handler_t *handlers = match_node(r->root, c->request.method, c->request.path, c);
    if (handlers) {
        c->handlers = handlers;
        c->handler_index = -1;
        return 1;
