...
graph TB
    subgraph types["fa:fa-plug 驱动类型"]
        AI["fa:fa-robot AI 驱动<br/>csilk_ai_driver_t<br/>chat, embeddings"]
        CIPHER["fa:fa-key Cipher 驱动<br/>csilk_cipher_driver_t<br/>encrypt, decrypt, sign, jwt"]
        DB["fa:fa-database DB 驱动<br/>csilk_db_driver_t<br/>connect, query, exec, txn"]
        PERM["fa:fa-shield Perm 驱动<br/>csilk_perm_driver_t<br/>check"]
        VECTOR["fa:fa-search 向量 DB 驱动<br/>csilk_vector_db_driver_t<br/>upsert, search"]
        CRYPTO["fa:fa-lock Crypto 驱动<br/>csilk_crypto_driver_t<br/>sha256, hmac, uuid, random, sha1, bcrypt"]
        STORAGE["fa:fa-hdd-o Storage 驱动<br/>csilk_storage_driver_t<br/>set, get, incr"]
    end

    subgraph registry["fa:fa-book 注册模式"]
        REG["fa:fa-list 全局静态注册表<br/>数组[DRIVER_MAX] + 计数"]
        INIT["fa:fa-rocket 首次使用延迟初始化<br/>注册内置默认驱动"]
        NEW["fa:fa-wrench 工厂: * _new(name, ...)<br/>查找 -> driver->init -> handle"]
        FREE["fa:fa-trash 析构: * _free(handle)<br/>driver->free(state) + free"]
    end

    subgraph backends["fa:fa-cogs 内置后端"]
        AI_BACKEND["fa:fa-cloud OpenAI, Ollama"]
        DB_BACKEND["fa:fa-database SQLite, MySQL,<br/>PostgreSQL, MongoDB, Redis"]
        PERM_BACKEND["fa:fa-check-circle 简单 RBAC"]
...
**内置后端**:
- **OpenAI** (`csilk_ai_openai_init_driver`): 通过 `libcurl` 的 HTTP REST API
- **Ollama** (`csilk_ai_ollama_init_driver`): 本地 HTTP API
...
sequenceDiagram
    actor 用户
    participant 注册表 as fa:fa-book 驱动注册表
    participant 驱动 as fa:fa-plug 具体驱动
    participant 句柄 as fa:fa-cog Opaque Handle

    Note over 用户: 初始化阶段
    用户->>注册表: csilk_db_init()
    注册表->>驱动: csilk_db_sqlite_init()
    驱动->>注册表: csilk_db_register_driver("sqlite", &driver)

    Note over 用户: 使用阶段
    用户->>注册表: csilk_db_pool_new("sqlite", "/data/db.sqlite")
    注册表->>注册表: find_driver("sqlite")
    注册表->>驱动: driver->connect(pool, dsn)
    驱动-->>注册表: csilk_db_pool_t* (wrapped connection)
    注册表-->>用户: handle

    用户->>句柄: csilk_db_pool_query(pool, "SELECT * FROM users")
    句柄->>驱动: driver->query(pool, sql, &result)
    驱动-->>用户: result (csilk_db_result_t)

    Note over 用户: 清理阶段
    用户->>句柄: csilk_db_pool_free(pool)
    句柄->>驱动: driver->disconnect(pool)
    句柄-->>用户: free(pool)
```
