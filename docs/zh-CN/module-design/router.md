...
flowchart TB
    subgraph param_types["fa:fa-tags 参数类型"]
        ...
        PARAM["fa:fa-hashtag PARAM: 命名捕获<br/>/users/:id<br/>  → result.params[0] = {':id', '42'}<br/>  then csilk_ctx_apply_route_result() arena-allocates"]
        WILD["fa:fa-asterisk WILDCARD: 贪婪后缀<br/>/static/*filepath<br/>  → result.params[0] = {'*filepath', 'css/app.css'}<br/>  then csilk_ctx_apply_route_result() arena-allocates"]
    end
```
