...
> **Version**: 0.5.0 | **Last updated**: 2026-06-27
...
```
1. Aggregated Counters      → http_requests_total_agg
                             → http_request_duration_microseconds_agg

2. D Dimensional Metrics     → http_requests_total{method,status,route}
...
3. Process Telemetry         → process_resident_memory_bytes
                             → process_cpu_seconds_total{type="user"}
                             → process_cpu_seconds_total{type="sys"}

4. Security Stats            → csilk_security_*_total

5. Server Connection Stats   → csilk_server_active_connections
                             → csilk_server_pooled_connections

6. MQ / AI / DB Stats        → csilk_mq_* (仅当 > 0)
...
| `src/app/admin.c` | Admin dashboard（读取 metrics 做实时可视化） |
