...
**失败路径**：Fase 1 或 2 中任何步骤失败 → `goto fail`，逆序释放已分配资源后返回 nullptr。
...
| `docs/module-design/server.md` | Server Core 设计文档 |
| `docs/module-design/router.md` | 路由设计文档 |
| `docs/module-design/middleware.md` | 中间件设计文档 |
...
- **封装性**：强制用户通过 API 函数交互，避免错误地直接修改内部状态（如 router->xxx）
...
- 允许多个 app 实例共享同一个 OpenAPI 端点（虽然实际使用中不常见）
