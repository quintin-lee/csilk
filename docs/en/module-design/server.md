...
> **Version**: 0.5.0 | **Last updated**: 2026-08-14
...
graph TB
    subgraph server_t["fa:fa-server csilk_server_t"]
        LOOP["fa:fa-sync-alt csilk_io_loop_t* loop<br/>Abstract event loop (libuv / io_uring)"]
        SRVH["fa:fa-plug csilk_io_tcp_t server_handle<br/>Listener socket"]
        SIG["fa:fa-bell csilk_io_signal_t sig_handle<br/>SIGINT handler"]
        ASYNC["fa:fa-bolt csilk_io_async_t async_handle<br/>Cross-thread wakeup"]
        CFG["fa:fa-cog csilk_server_config_t config<br/>Timeouts, TLS, workers, backpressure..."]

        MW["fa:fa-layer-group csilk_handler_t middlewares[32]<br/>Global middleware chain"]
        RT["fa:fa-sitemap csilk_router_t* router<br/>Segment-based prefix trie router"]
        ...
        ATOMIC["fa:fa-chart-bar atomic_int active_connections<br/>Connection counter (Atomic CAS reservation)"]
    end
...
| `router` | `csilk_router_t*` | 路由表（Segment-based Prefix Trie） |
...
- 设置默认超时和最大尺寸（头部 64KB、URL 8KB、请求体 1MB、背压高水位 64KB、低水位 16KB、最大缓冲区 16MB）
- 关联路由器实例
- 设置 llhttp 解析器回调
- 创建内部消息队列
...
flowchart TB
    ...
    WORKERS -->|Yes| SPAWN["fa:fa-cogs csilk_thread_create × N<br/>&#10;SO_REUSEPORT per worker"]
    WORKERS -->|No| SINGLE["fa:fa-user Single worker<br/>Direct csilk_io_run()"]

    SPAWN --> MAIN["fa:fa-crown Main thread csilk_io_run()"]
...
sequenceDiagram
    ...
    Server->>Timer: stop idle, start read timer
    Client->>Server: ...sending body...
    Server->>Timer: restart read timer
    Client->>Server: Request complete
    Server->>Timer: stop read, start request timer
    Server->>Client: Response
    Server->>Timer: stop all, start idle timer
    Note over Server,Timer: Connection: keep-alive → loop back
```
