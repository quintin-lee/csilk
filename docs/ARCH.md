...
Server-C 采用了经典的 **Reactor 事件驱动模型**，结合了类似 Go 语言 Gin 框架的 **洋葱模型 (Onion Model)** 中间件机制。
...
- 请求生命周期内的所有临时字符串（如 `gin_string`）、URL 参数、解析出的 Headers 均从 Arena 中分配。
...
int main() {
    ...
    gin_group_t* g = gin_group_new(r, "/api");
    gin_GET(g, "/ping", handler);
    
    gin_server_t* s = gin_server_new(r);
    gin_server_run(s, 8080);
    return 0;
}
```
